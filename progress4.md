...
> Paste this file together with agent4.md at the start of every new chat.
        ...
        → exams/    (requires questions + accounts)
          → dashboard/  (reads from exams/, comes last)
            → org/      (reads from everything, comes last)
              → Quick Wins / New Question Types / Section-Type Refactor / Passage / Rework / Scoring / Org Dashboard
                → Polish & Deploy (final stage)
```

> **Note:** `students/` app removed from build order — student tracking is out of scope.
> **Note:** Stage 9.5 is new — inserted between Stage 9 and Stage 10 after a design review. Do not skip it: Stage 10 (Passage) is built assuming `question_type`/`section` already exist as real model fields, not tags.
...
**Check:** Fill in the Blank sorusu şıksız kaydedilebiliyor. Matching sorusu çift olarak kaydedilip PDF'te iki sütun halinde, sağ taraf karışık sırayla görünüyor. ✅ Verified.

---

## Stage 9.5 — Section & Question Type Refactor

> **New stage, added after a design review at the end of Stage 9.** Before starting Stage 10 (Passage), the old approach of encoding both "question type" (MC/FIB/Matching) and "section" (Reading/Listening/Speaking) as `Tag` objects in the same M2M field needs to be replaced with two explicit, independent model fields. See `agent4.md` → "Design Decision Log" for the full reasoning.
>
> **Why this can't wait until Stage 10:** Passage's whole purpose is to group questions under a shared Reading/Listening section. If `section` isn't a real field yet, Passage would have to invent its own ad-hoc way of tagging questions as Reading/Listening, duplicating work that belongs here. Do this first.

### 9.5.1 Model Changes
- [x] Add `Question.question_type` → `CharField(choices=[mc, tf, fib, matching, open_ended])`
- [x] Add `Question.section` → `CharField(choices=[general, reading, listening, writing, speaking], default="general")`
- [x] Add `Question.word_count_instruction` → `CharField(max_length=100, null=True, blank=True)` — free-text instruction for `open_ended` questions (e.g. "Write at least 150 words"), no correct answer stored, no min/max integer validation needed
- [x] Migration for the three new fields

### 9.5.2 Data Migration (Backfill Existing Questions)
- [x] Write a data migration that inspects each existing `Question`'s tags and sets `question_type`:
  - Has "Fill in the Blank" tag → `question_type=fib`
  - Has "Matching" tag → `question_type=matching`
  - Has 2 `Choice` rows → `question_type=tf`
  - Has 4 (or more) `Choice` rows → `question_type=mc`
- [x] All existing questions get `section=general` by default (no Reading/Listening/Speaking/Writing existed before this stage)
- [x] After confirming the migration ran correctly, remove "Fill in the Blank" / "Matching" from the `Tag` table (or leave them — decide case by case; they are no longer read by any code path either way)

### 9.5.3 `ENABLED_SECTIONS` Setting
- [x] Add `ENABLED_SECTIONS = ["general", "reading", "listening", "writing", "speaking"]` to `settings/base.py`
- [x] Add `enabled_sections` context processor in `core/context_processors.py`, register in `settings/base.py`
- [x] `QuestionForm`'s `section` field choices filtered to `ENABLED_SECTIONS` in `__init__`
- [x] Any section-picker UI (question list filters, exam generation form later) also filters by `ENABLED_SECTIONS`

### 9.5.4 Form Changes
- [x] `QuestionForm`: `section` and `question_type` become two independent dropdowns (not derived from tags)
- [x] JS: showing/hiding `ChoiceFormSet` / `correct_answer_text` field / `MatchingPairFormSet` / `word_count_instruction` field now branches on the selected `question_type` value, not on tag name string-matching
- [x] `open_ended` selection hides all choice/answer-key-related fields and shows only `text` (used as the prompt) + `word_count_instruction`

### 9.5.5 Rendering Changes
- [x] `preview.html` / `pdf.html`: branch on `question.question_type` instead of checking tag names
- [x] `open_ended` questions render as: prompt text + word count instruction (if set) + blank ruled lines
- [x] `open_ended` questions are excluded from the PDF answer key page (no correct answer to show)

**Check:** Every existing question still renders correctly after the data migration (spot-check a handful of MC, True/False, Fill in the Blank, and Matching questions). New questions can be created with `section=writing` or `section=speaking` and `question_type=open_ended`, rendering a blank writing area with no answer key entry. Removing `"speaking"` from `ENABLED_SECTIONS` hides it from the contributor form and question list without deleting any data; adding it back restores it.
...
- [ ] `Passage` modeli:
  ...
  - 'image', 'image_label' (image için, null=True)
  - `created_by` FK
- [x] `Question.passage` → yeni `ForeignKey(Passage, null=True, blank=True, related_name="questions")`
- [x] `Question.level` ve `Question.section` alt sorularda artık elle girilmeyecek — `passage.level` ve `passage.type`'dan (`reading`/`listening`) otomatik miras alınır (view'da `form_valid()` içinde set edilir, formda hiç gösterilmez)
- [x] Migration
...
> Stage 9.5, 10 ve 11 tamamlanmadan başlanamaz — "section" kavramı Stage 9.5'ten geliyor.

- [ ] Sınav oluşturma formunda, her section/question_type kombinasyonu (general MC, general TF, Fill in the Blank, Matching, Reading, Listening, Writing, Speaking) için puan girişi
...
- [ ] `OrgDashboardView`'a ek istatistikler: section bazlı soru sayısı (general/reading/listening/writing/speaking), paragraf sayısı, listening parça sayısı, question_type bazlı soru sayısı (MC / TF / Fill in the Blank / Matching), toplam havuz büyüklüğü
...
- [ ] `open_ended` (writing/speaking) questions correctly excluded from the answer key?
- [ ] Disabling a section via `ENABLED_SECTIONS` hides it everywhere without deleting data?
...
| 9.5 | Section & Question Type Refactor | 9 |
| 10 | Passage modeli | 9.5 |
| 11 | Exam generation rework | 4, 10 |
| 12 | Section bazlı puanlama | 9.5, 11 |
| 13 | Org dashboard zenginleştirme | 6, 9.5 |
...
- [x] **NOT (Stage 9.5 ile netleşti):** Eski yapıda "question type" (MC/FIB/Matching) ve "section" (Reading/Listening/Speaking) aynı `Tag` M2M alanında karışık tutuluyordu, bu da örneğin bir sorunun hem "Speaking" hem "Reading" olarak işaretlenebilmesine izin veriyordu (mantıksız kombinasyon, DB seviyesinde engellenmiyordu). Stage 9.5'te bu ikisi `Question.question_type` ve `Question.section` olarak ayrı, birbirinden bağımsız CharField'lara taşındı.
- [x] **NOT (Stage 10 ile netleşti):** Passage'ın "3-5 alt soru" sınırı kaldırıldı — sabit sayı yerine "+ Soru Ekle" akışıyla serbest sayıda (min 1) soru eklenebiliyor, alt sorular farklı question_type'larda olabiliyor.
- [ ] **NOT (Stage 11 öncesi netleştirilmeli):** Bir paragrafın soruları seviye sayımına (level_counts) dahil ediliyor — bu netleşti. Ama grup içi soru sırasının (paragraf içindeki 4 sorudan 2'si seçildiğinde) kendi aralarında karışıp karışmayacağı netleşmedi, sadece "paragraflar arası ardışıklık" netleşti — bu detay Stage 11 tasarımı sırasında teyit edilecek.
- [ ] **NOT (Beklemede):** Speaking section'ının kalıcı olarak projede tutulup tutulmayacağı ekip arkadaşıyla netleşmedi. Şimdilik `settings.ENABLED_SECTIONS` listesinde açık — karar netleşince listeden çıkarılabilir, model/migration değişikliği gerekmez.
